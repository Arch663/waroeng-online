...
function draw() {
  ...
  const labels = props.points.map((p) => p.category || t("common_stock"));
...
<template>
  <div class="p-5 md:p-8 h-full flex flex-col">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h3 class="text-xs font-black text-muted uppercase tracking-widest mb-1">{{ t("dashboard_chart_category_split") }}</h3>
        <h2 class="text-xl md:text-2xl font-black text-foreground uppercase tracking-tight">{{ t("dashboard_chart_category_sales") }}</h2>
      </div>
