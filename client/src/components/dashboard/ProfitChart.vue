...
function draw() {
  ...
  const config: any = {
    ...
    data: {
      ...
      datasets: [
        {
          label: t("dashboard_chart_net_profit"),
          data,
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            label(context: any) {
              return `${t("dashboard_chart_net_profit")}: Rp ${context.parsed.y.toLocaleString("id-ID")}`;
            },
          },
        },
      },
...
<template>
  <div class="p-8 h-80 flex flex-col">
    <div class="flex items-center justify-between mb-8">
      <div>
        <h3 class="text-xs font-black text-muted uppercase tracking-widest mb-2">{{ t("dashboard_chart_efficiency") }}</h3>
        <h2 class="text-2xl font-black text-foreground uppercase tracking-tight">{{ title || t("dashboard_chart_profit_analysis") }}</h2>
      </div>
      <div class="w-10 h-10 flex items-center justify-center bg-accent/10 text-accent rounded-xls">
        <svg
          class="w-5 h-5"
          viewBox="0 0 24 24"
          fill="none"
